...
我们将使用Homebrew安装GCC工具链。
Homebrew是MacOS上的一个社区运营的包管理器，可以用来方便地下载和管理很多软件，尤其是与软件开发的一些软件。
...
> 为什么我们不推荐使用`xcode-select`安装的Apple Clang编译器？
> 
> Apple Clang编译器是Apple公司对Clang编译器的修改品，相比于原版有很多现代c++特性不支持，这并不是我们想要的。

安装Homebrew，只需要在终端中运行以下命令（可能需要比较“良好”的网络环境）：

> 如果你的网络没那么“好”，可以参考[清华大学镜像站Homebrew镜像](https://mirrors.tuna.tsinghua.edu.cn/help/homebrew/)
...
（如果没有`gcc-13`命令，可能是gcc版本已经更新，可以尝试`gcc-14`等）

你也可以将`gcc`修改成`gcc-13`的别名，以方便使用：

- 如果你的Mac电脑是使用M系列的arm芯片（较新的Mac电脑），运行下述命令，然后重启终端即可
...
- 如果你的Mac电脑是使用较老的Intel芯片，运行下述命令，然后重启终端即可
